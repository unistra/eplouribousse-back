...
class UserListTest(TestCase):
    ...
    def test_user_must_be_authenticated(self):
        response = self.get(reverse("user-list"))
        self.response_unauthorized(response)

    def test_list_users(self):
        response = self.get(reverse("user-list"), user=self.user)
        self.response_ok(response)
        self.assertEqual(response.data["count"], 11)

    def test_inactive_user_is_not_returned(self):
        response = self.get(reverse("user-list"), user=self.user, data={"page_size": 20})
        self.response_ok(response)
        self.assertEqual(response.data["count"], 11)
        self.assertNotIn("inactive@eplouribousse.fr", [user["email"] for user in response.data["results"]])

    def test_search_user(self):
        response = self.get(reverse("user-list"), user=self.user, data={"search": "user@eplouribousse.fr"})
        self.response_ok(response)
    ...
    def test_search_does_not_return_inactive_user(self):
        response = self.get(reverse("user-list"), user=self.user, data={"search": "inactive@eplouribousse.fr"})
        self.response_ok(response)

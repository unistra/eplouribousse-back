...
class UserRoleModelTest(TenantTestCase):
    def setUp(self):
        ...
        self.role1 = UserRole.objects.create(
            user=self.user1, project=self.project1, role=ProjectRole.INSTRUCTOR, assigned_by=self.admin
        )
        self.role2 = UserRole.objects.create(user=self.user2, project=self.project1, role=ProjectRole.PROJECT_CREATOR)

    def test_userrole_creation(self):
        """Tests basic creation of a UserRole"""
        role = UserRole.objects.create(user=self.user1, project=self.project2, role=ProjectRole.GUEST)
        self.assertEqual(role.user, self.user1)
        self.assertEqual(role.project, self.project2)
        self.assertEqual(role.role, ProjectRole.GUEST)
    ...
    def test_userrole_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            UserRole.objects.create(user=self.user1, project=self.project1, role=ProjectRole.INSTRUCTOR)
    ...
    def test_userrole_unique_constraint_different_users_can_have_same_role(self):
        ...
        self.assertEqual(created_role.role, ProjectRole.INSTRUCTOR)
    ...
    def test_userrole_unique_constraint_same_user_can_have_different_roles(self):
        ...
        self.assertEqual(created_role.project, self.project1)
        self.assertEqual(created_role.role, ProjectRole.PROJECT_CREATOR)
